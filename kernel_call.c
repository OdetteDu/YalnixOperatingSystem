...
extern int KernelExit(int status)
{
  ...
  if(active_process->parent == 0){
    ...
    if(statusQ ==NULL){//simply initialize the statusQ
      (active_process->parent)->exitStatusQ = newStatus;
    }else{//put the new item in the end of the status q
      while(statusQ->next != NULL){//get to the end of Q
...
extern int KernelWait(int *status_ptr)
{
  ...
  if(children == NULL){//no child for parent
    return ERROR;
  }
